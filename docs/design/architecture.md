...
**Monorepo, not separate frontend/backend repos.** Splitting a repo is the expensive, mostly-irreversible move — it costs separate CI, separate release coordination, and tooling duplicated across two places to undo later. Staying together is the default, and nothing here has earned the cost of splitting: the two apps deploy together, there's no independent release cadence or access-control boundary that needs enforcing, and no second team exists to justify separating ownership. If one of those becomes true, that's the trigger to reconsider — this isn't a plan already in motion, it's the absence of a reason to move.

## Alternatives considered

**Separate repositories for frontend and backend, rejected.** The burden of proof sits on splitting, not on staying together — separate repos mean separate CI, separate release coordination, and tooling duplicated across two places, all to buy independent deploy cadences and independent access control that nobody needs yet on a single small team shipping both apps together.
